bph.accountHandling = (function () { 
	...
	function _getUsernameCode(username) { 
		return "document.getElementById(\"i0116\").value = \"" + username + "\";";
	}

	function _getPasswordCode(password) { 
		return "document.getElementById(\"i0118\").value = \"" + password + "\";";
	}

	function _getPressLoginButtonCode() { 
		return "document.getElementById(\"idSIButton9\").click();";
	}

	function _inputLoginDetails(username, password, loginWindow, loginTab, callback) { 
		var usernameTimeout, passwordTimeout, buttonClickTimeout;

		var inputUsername = function () {
			chrome.tabs.executeScript(loginTab.id, {code: _getUsernameCode(username), runAt: "document_idle"}, function (input) {
				clearTimeout(usernameTimeout);
					
				var inputPassword = function () {
					chrome.tabs.executeScript(loginTab.id, {code: _getPasswordCode(password), runAt: "document_idle"}, function (input) { 
						clearTimeout(passwordTimeout);
							
						var pressLoginButton = function () {
							chrome.tabs.executeScript(loginTab.id, {code: _getPressLoginButtonCode(), runAt: "document_idle"}, function (input) {
								setTimeout(function () {
